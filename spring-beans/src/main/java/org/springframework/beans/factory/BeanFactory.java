...
public interface BeanFactory {
	...
	/**
	 * 区分于普通的bean和FactoryBean创建的bean。
	 * - 当一个bean是FactoryBean类型时（比如名为"myJndiObject"的bean）
	 * - 直接调用 getBean("myJndiObject") 会返回FactoryBean创建的实例
	 * - 而调用 getBean("&myJndiObject") 则会返回FactoryBean本身
	...
	/**
	 * 根据指定的bean名称和对应的构造参数列表/工厂方法参数列表来获取对应的实例
	 * Return an instance, which may be shared or independent, of the specified bean.
	 ...
	 * 对于匹配泛型类型,请考虑{@link #getBeanProvider(ResolvableType)}。
